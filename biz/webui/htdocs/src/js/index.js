...
var Index = React.createClass({
  getInitialState: function () {
    ...
    if (!pageName || pageName.indexOf('rules') != -1) {
      ...
      state.name = 'plugins';
    } else if (pageName.indexOf('composer') != -1) {
      state.hasComposer = true;
      state.name = 'composer';
    } else if (pageName.indexOf('tools') != -1) {
      state.hasToolbox = true;
      state.name = 'tools';
    } else if (pageName.indexOf('crypto') != -1) {
      state.hasCrypto = true;
      state.name = 'crypto';
    } else if (pageName.indexOf('codec') != -1) {
      state.hasCodec = true;
      state.name = 'codec';
    } else if (pageName.indexOf('json') != -1) {
      state.hasJson = true;
      state.name = 'json';
  ...
  componentDidMount: function () {
    ...
    events.on('showComposer', function (_, data) {
      self.setState({
        composeModel: data
      })
      self.showComposer();
  ...
  showComposer: function (e) {
    ...
    util.changePageName('composer');
  },
  /* 显示工具 */
  showToolbox: function (e) {
    if (this.state.name != 'tools') {
      this.setMenuOptionsState();
      this.hideRulesOptions();
      this.hidePluginsOptions();
      this.hideValuesOptions();
    ...
    util.changePageName('tools');
  },
  /* 显示加密 */
  showCrypto: function (e) {
    if (this.state.name != 'crypto') {
      this.setMenuOptionsState();
      this.hideRulesOptions();
      this.hidePluginsOptions();
      this.hideValuesOptions();
  ...
  showCodec: function (e) {
    ...
    util.changePageName('codec');
  },
  /* 显示json编辑器 */
  showJson: function (e) {
    if (this.state.name != 'json') {
      this.setMenuOptionsState();
      this.hideRulesOptions();
      this.hidePluginsOptions();
      this.hideValuesOptions();
    ...
    util.changePageName('json');
  },
  /* 显示js编辑器 */
  showJs: function (e) {
    if (this.state.name != 'js') {
      this.setMenuOptionsState();
      this.hideRulesOptions();
      this.hidePluginsOptions();
      this.hideValuesOptions();
  ...
  _uploadRules: function (data, showResult) {
    ...
    dataCenter.upload.importRules(data, function (data, xhr) {
      if (!data) {
        ...
        self.reloadRules(data);
        showResult && message.success('成功的同步规则.');
      } else {
        win.alert(data.em);
      }
    });
  ...
  _uploadValues: function (data, showResult) {
    ...
    dataCenter.upload.importValues(data, function (data, xhr) {
      ...
      if (data.ec === 0) {
        self.reloadValues(data);
        showResult && message.success('成功同步值.');
      } else {
        win.alert(data.em);
      }
    });
  ...
  uploadRules: function (e) {
    ...
    if (!file || !/\.(txt|json)$/i.test(file.name)) {
      return win.alert('只支持 .txt 或者 .json 文件.');
    }

    if (file.size > MAX_OBJECT_SIZE) {
      return win.alert('文件大小不能超过6m.');
    }
  ...
  confirmDisableAllRules: function (e) {
    ...
    if (state.disabledAllRules) {
      ...
      win.confirm('你确定要禁用所有规则吗?', function (sure) {
  ...
  uploadSessionsForm: function (data) {
    ...
    if (!file || !/\.(txt|json|saz|har)$/i.test(file.name)) {
      return win.alert('只支持 .txt, .json, .saz  .har 等文件.');
    }

    if (file.size > MAX_FILE_SIZE) {
      return win.alert('文件大小不能超过64m.');
    }
        ...
        <div className={'w-menu w-' + name + '-menu-list'}>
          ...
          {!state.ndr && (
            <a
              ...
              title={
                disabledAllRules ? '启用所有规则' : '禁用所有规则'
              }
              style={{
          ...
          <a
            ...
            style={{ display: isNetwork || isRules || isValues ? '' : 'none' }}
            draggable="false"
          >
            <span className="glyphicon glyphicon-import"></span>导入
          </a>
          {/* 导出 插件专用*/}
          <a
            ...
            draggable="false"
          >
            <span className="glyphicon glyphicon-export"></span>导出
          </a>
          {/* 清除 网络专用*/}
          <div
          ...
          >
            <a
              ...
              draggable="false"
            >
              <span className="glyphicon glyphicon-remove"></span>清除
            </a>
            <MenuItem
          ...
          <a
            ...
            style={{ display: isRules || isValues ? '' : 'none' }}
            draggable="false"
            title="Ctrl[Command] + S"
          >
            <span className="glyphicon glyphicon-save-file"></span>保存
          </a>
          {/* 创建 网络|插件不能用*/}
          <a
          ...
          >
            <span className="glyphicon glyphicon-plus"></span>创建
          </a>
          {/* 重命名 */}
          <a
            onClick={this.onClickMenu}
            ...
            draggable="false"
          >
            <span className="glyphicon glyphicon-edit"></span>重命名
          </a>
          {/* 重播 */}
          <div
          ...
          <a
            ...
            draggable="false"
          >
            <span className="glyphicon glyphicon-edit"></span>请求
          </a>
          {/* 删除 */}
          <a
            onClick={this.onClickMenu}
          ...
          <FilterBtn
            ...
            isNetwork={isNetwork}
            hide={!isNetwork && !isRules && !isValues}
          />
          {/* 文件 */}
          <a
            ...
            draggable="false"
          >
            <span className="glyphicon glyphicon-upload"></span>文件
          </a>
          {/* Weinre */}
          <div
          ...
          >
            ...
            >
              <span className="glyphicon glyphicon-question-sign"></span>帮助
            </a>
            <MenuItem
              ref="helpMenuItem"
        ...
        <div className="w-container box fill">
          ...
          >
            ...
            >
              ...
              <i className="w-left-menu-name">网络</i>
            </a>
            {/* 规则 */}
            <a
            ...
            >
              ...
              ></span>
              <i className="w-left-menu-name">规则</i>
              <i
            ...
            >
              ...
              <i className="w-left-menu-name">数据</i>
              <i
                className="w-menu-changed"
                style={{
            ...
            >
              ...
              ></span>
              <i className="w-left-menu-name">插件</i>
            </a>
            {/* 请求 */}
            <a
              ...
              className={
                'w-save-menu w-values-menu' +
                (name == 'composer' ? ' w-menu-selected' : '')
              }
              draggable="false"
            >
              <span className="glyphicon glyphicon-edit"></span>
              <i className="w-left-menu-name">请求</i>
            </a>
            {/* 工具 */}
            <a
              ...
              className={
                'w-save-menu w-values-menu' +
                (name == 'tools' ? ' w-menu-selected' : '')
              }
              draggable="false"
            >
              <span className="glyphicon glyphicon-wrench"></span>
              <i className="w-left-menu-name">工具</i>
            </a>
            {/* 加密 */}
            <a
              onClick={this.showCrypto}
              className={
                'w-save-menu w-values-menu' +
                (name == 'crypto' ? ' w-menu-selected' : '')
              }
              draggable="false"
            >
              <span className="glyphicon glyphicon-lock"></span>
              <i className="w-left-menu-name">加密</i>
            </a>
            {/* 编码 */}
            <a
              onClick={this.showCodec}
              className={
                'w-save-menu w-values-menu' +
                (name == 'codec' ? ' w-menu-selected' : '')
              }
              draggable="false"
            >
              <span className="glyphicon glyphicon-transfer"></span>
              <i className="w-left-menu-name">编码</i>
            </a>
            {/* json */}
            <a
              ...
              className={
                'w-save-menu w-values-menu' +
                (name == 'json' ? ' w-menu-selected' : '')
              }
              draggable="false"
            >
              <span className="glyphicon glyphicon-sort-by-attributes"></span>
              <i className="w-left-menu-name">JSON</i>
            </a>
            {/* js */}
            <a
              onClick={this.showJs}
          ...
          />}
          {state.hasNetwork && <Network
            ref="network"
            hide={name == 'network' ? false : true}
            modal={modal}
          />}
          {state.hasPlugins && <Plugins
            {...state}
          ...
          />}
          {state.hasComposer && <ComposerList
            modal={state.composeModel}
            hide={name == 'composer' ? false : true}
          />}
          {state.hasToolbox && <ToolBox hide={name == 'tools' ? false : true} /> }
          {state.hasCrypto && <Container url="./tools/crypto/index.html#crypto" hide={name == 'crypto' ? false : true} />}
                ...
                <div className="w-https-settings">
                  <p>
                    <label
                      title={
                        multiEnv
                          ? '使用`pattern enable://capture`在规则中替换全局配置'
                          : undefined
                      }
                    >
                      <input
            ...
            <div className="modal-content">
              <div className="modal-body">
                ...
                <p className="w-show-update-tips">
                  whistle 有重要更新,建议您更新至最新版本.
                </p>
                <p>当前版本: {state.version}</p>
                <p>最新的稳定版本: {state.latestVersion}</p>
                <p>
                  视图更改:{' '}
                  <a
                    title="更改日志"
